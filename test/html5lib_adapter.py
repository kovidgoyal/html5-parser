...
# The tree construction tests are no longer maintained in html5lib-tests, they
# now live in web-platform-tests, see
# https://github.com/html5lib/html5lib-tests/commit/224991e
wpt_tree_construction_path = os.path.join(
    base, 'wpt', 'html', 'syntax', 'parsing', 'resources')
...
class TestData(object):
    ...
    def __init__(self, filename):
        ...
        # Tests are separated by a blank line and the file is supposed to end
        # with a single newline, but not every .dat file gets that right, and a
        # trailing blank line would otherwise end up in the last test's
        # expected output.
...
def serialize_construction_output(root, fragment_context):
    ...
    def serialize_pi(node, level=1):
        add(level, '<?', node.target, ' ', node.text or '', '?>')

    def serialize_non_element(node, level=1):
        if isinstance(node, _ProcessingInstruction):
            serialize_pi(node, level)
        else:
            serialize_comment(node, level)
    ...
    def serialize_node(node, level=1):
        ...
        for child in node:
            if isinstance(child, (_Comment, _ProcessingInstruction)):
                serialize_non_element(child, level + 2)
            else:
    ...
    if fragment_context:
        ...
        for node in root.iterchildren():
            if isinstance(node, (_Comment, _ProcessingInstruction)):
                serialize_non_element(node)
            else:
                serialize_node(node)
            if node.tail:
...
class BaseTest(TestCase):
    ...
    def add_single(cls, test_name, num, test, expected):
        ...
        # Tests that need scripting enabled can never pass, gumbo always parses
        # with scripting disabled.
        scripted = 'script-on' in test
        ...
        def test_func(
                ...
                scripted=scripted):
            if scripted:
                raise unittest.SkipTest('gumbo always parses with scripting disabled')
...
class ConstructionTests(BaseTest):
    ...
    def check_test(cls, fragment_context, html, expected, errors, test_name):
        ...
        if '<selectedcontent>' in expected:
            return (
                '<selectedcontent> is kept in sync with the contents of the selected'
                ' <option> by the DOM, not by the parser, and gumbo is only a parser')
        for line in errors:
    ...
    def implementation(self, fragment_context, html, expected, errors, test_name):
        ...
        if expected != output and expected.lower() == output.lower():
            raise unittest.SkipTest(
                'gumbo output differs from html5lib only in letter case, because'
                ' gumbo does not lowercase unknown tag names')
...
def html5lib_test_files(group):
    if group == 'tree-construction':
        base = wpt_tree_construction_path
    else:
