...
static void reconstruct_active_formatting_elements(GumboParser* parser) {
  ...
  for(unsigned int c=i; c < elements->length; ++c) {
    ...
    GumboNode* clone = clone_node(
        element, GUMBO_INSERTION_RECONSTRUCTED_FORMATTING_ELEMENT);
    // Step 9.
    InsertionLocation location = get_appropriate_insertion_location(parser, NULL);
  ...
  for (int i = 0; i < 8; ++i) {
    ...
    for (int j = 0;;) {
      ...
      if (j > 3 && formatting_index != -1) {
        ...
        gumbo_vector_remove_at(formatting_index,
      ...
      if (last_node == furthest_block) {
        ...
        assert(UINT_MAX + bookmark <= state->_active_formatting_elements.length);
      }
      // Step 13.9.
    ...
    gumbo_vector_remove_at(formatting_node_index, &state->_active_formatting_elements);
    assert(bookmark >= 0);
    ...
    gumbo_vector_insert_at(new_formatting_node, insert_at, &state->_open_elements);
  } // Step 20.
  return true;
}
      ...
      case GUMBO_TOKEN_START_TAG:
        switch (token->v.start_tag.tag) {
            ...
            case GUMBO_TAG_TEXTAREA:
                if (parser->_options->use_xhtml_rules && token->v.start_tag.is_self_closing) {
                    set_frameset_not_ok(parser);
                    insert_element_from_token(parser, token);
                    return true;
                }
      ...
      case GUMBO_TOKEN_END_TAG:
        switch (token->v.end_tag) {
            ...
            case GUMBO_TAG_H6: {
                ...
                if (!success) {
                    // There are children of the heading currently open; close them below and
