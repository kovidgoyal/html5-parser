...
static void handle_parser_error(
  ...
  if (error->parser_state == GUMBO_INSERTION_MODE_INITIAL &&
    ...
    print_message(
        output, "The doctype must be the first token in the document");
    return;
  }
...
GumboError* gumbo_add_error(GumboParser* parser) {
  ...
  if (parser->_options->max_errors >= 0 &&
      parser->_output->errors.length >= max_errors) {
    return NULL;
  }
...
void gumbo_error_to_string(const GumboError* error, GumboStringBuffer* output) {
  print_message(
      output, "@%d:%d: ", error->position.line, error->position.column);
  switch (error->type) {
    case GUMBO_ERR_UTF8_INVALID:
      print_message(output, "Invalid UTF8 character 0x%x", error->v.codepoint);
      break;
    ...
    case GUMBO_ERR_NUMERIC_CHAR_REF_NO_DIGITS:
      print_message(
          output, "No digits after &# in numeric character reference");
      break;
    ...
    default:
      print_message(
          output, "Tokenizer error with an unimplemented error message");
      break;
  }
  gumbo_string_buffer_append_codepoint('.', output);
}
